...
hyperparameters = GridSearch.FNNMC.Hyperparameters(learningRate=0.001, inputSize=underlyingsTotal, hiddenlayer1=underlyingsTotal+1, epochs=10, batchSize=10**4, trainOnlyLastTimeStep=True)
timeRegressionStart = time.time()
GridSearch.FNNMC.findNeuralNetworkModels(simulatedPaths=learningPaths, Option=callGeometricAverage, MarketVariables=marketVariables, hyperparameters=hyperparameters)
...
for i in range(100):
    ...
    price = GridSearch.FNNMC.priceAmericanOption(simulatedPaths=pricingPaths, Option=callGeometricAverage, MarketVariables=marketVariables, hyperparameters=hyperparameters)*normalizeStrike
