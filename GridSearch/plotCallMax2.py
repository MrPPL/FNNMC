...
hiddenLayers = pd.DataFrame(data, columns=['Hidden Layers']).to_numpy()
activationFunc = pd.DataFrame(data, columns=['Activation Function']).to_numpy()
price1 = price[np.logical_and(hiddenLayers==4, activationFunc=='Relu')]
price2 = price[np.logical_and(hiddenLayers==4, activationFunc=='Leaky (a=0.01)')]
price3 = price[np.logical_and(hiddenLayers==4, activationFunc=='Leaky (a=0.3)')]
width1 = activationFunc[activationFunc=='Relu']
width2 = activationFunc[activationFunc=='Leaky (a=0.01)']
width3 = activationFunc[activationFunc=='Leaky (a=0.3)']
plt.style.use('ggplot')
plt.plot(width, price, '+', color='Blue', alpha=0.5)
plt.plot(width,[13.902]*48, '-', label='Benchmark Price')
plt.legend()
plt.title('Grid Seach Call Max on Two Assets')
plt.xscale('log',base=10) 
plt.xlabel('Width')
